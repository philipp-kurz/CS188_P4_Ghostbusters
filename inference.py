...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            pObs += obsProb
        for ghostPosition in self.allPositions:
            self.beliefs[ghostPosition] /= pObs
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newPosDists = {}
        predicts = DiscreteDistribution()

        for oldPos in self.allPositions:
            newPosDists[oldPos] = self.getPositionDistribution(gameState, oldPos)

        for newPos in self.allPositions:
            for oldPos in self.allPositions:
                predicts[newPos] += newPosDists[oldPos][newPos] * self.beliefs[oldPos]

        for pos in self.allPositions:
            self.beliefs[pos] = predicts[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            if weightDist[particle] == 0:
                numZeroEntries += 1

        if numZeroEntries == len(self.particles):
            self.initializeUniformly(gameState)
        else:
            for i in range(len(self.particles)):
                self.particles[i] = weightDist.sample()
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            ...
            newParticles.append(newPostDist.sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        possibleParticles = list(itertools.product(self.legalPositions, self.legalPositions))


        for i in range(self.numParticles):
            self.particles.append(possibleParticles[i % len(possibleParticles)])
